...
if __name__ == "__main__":
    ...
    if not daily_volatility_df.empty:
        ...
        daily_volatility_df['is_stable'] = daily_volatility_df['每日标准差'] < stable_threshold
        
        # 使用 groupby 和 shift() 来找出所有连续的平稳期
        stable_streaks = daily_volatility_df[daily_volatility_df['is_stable']].groupby(
            (daily_volatility_df['is_stable'] != daily_volatility_df['is_stable'].shift()).cumsum()
        ).size()
        ...
        if not stable_streaks.empty:
            ...
            longest_streak_start_date = daily_volatility_df[
                (daily_volatility_df['is_stable']) & (
                    (daily_volatility_df['is_stable'] != daily_volatility_df['is_stable'].shift()).cumsum() == longest_streak_id
                )
            ].index.min()
