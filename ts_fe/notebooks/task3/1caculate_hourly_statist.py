...
def get_humidity_data(start_time_utc_str: str, stop_time_utc_str: str) -> pd.DataFrame:
    ...
    with InfluxDBClient(
        ...
        print(
            f"  正在从 InfluxDB 查询从 {start_time_utc_str} 到 {stop_time_utc_str} 的数据..."
        )
...
if __name__ == "__main__":
    ...
    for year in range(start_year, end_year + 1):
        ...
        # b. 提取这一年的原始数据
        yearly_df = get_humidity_data(
            start_time_utc_str=year_start_utc_str, stop_time_utc_str=year_end_utc_str
        )
        ...
        if last_processed_time and not hourly_features.empty:
            hourly_features = hourly_features[
                hourly_features.index > last_processed_time
            ]
