...
from src.features.statistica import recalculate_single_hour_features
...
def test_feature_calculation_correctness():
    ...
    # 从标准答案中获取时间戳
    timestamp_utc = pd.to_datetime(stored_features_df["stats_start_time"].iloc[0]).tz_localize(
        "UTC"
    )
