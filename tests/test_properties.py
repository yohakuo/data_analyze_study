...
# ============================================================================
# Statistical features completeness
# Feature: feature-calculator-mcp
# ============================================================================
...
@given(
    ...
    field_name=st.text(
        min_size=1, max_size=20, alphabet=st.characters(blacklist_characters=["\x00"])
    ),
...
def test_property_1_statistical_features_completeness(num_rows, field_name, feature_list, freq):
    ...
    **Validates: Requirements 1.1**
    ...
    df = pd.DataFrame({field_name: np.random.uniform(10, 100, num_rows)}, index=index)
...
@given(
    ...
    feature_list=st.lists(
        st.sampled_from(["均值", "最大值", "最小值"]), min_size=1, max_size=3, unique=True
    ),
...
def test_property_3_automatic_range_calculation(
    num_rows, field_name, include_other_features, freq
):
    """
    ...
    **Validates: Requirements 1.3, 1.4**
    ...
    assert "极差" in result.columns, (
        "极差 column should be automatically added when both max and min are requested"
    )

    # Check that 极差的时间变化率 column exists
    assert "极差的时间变化率" in result.columns, (
        "极差的时间变化率 column should be automatically added when range is calculated"
    )

    # Verify that 极差 = 最大值 - 最小值 for all rows
    calculated_range = result["最大值"] - result["最小值"]
    pd.testing.assert_series_equal(result["极差"], calculated_range, check_names=False, atol=1e-10)

    # Verify that 极差的时间变化率 is the percentage change of 极差
    expected_rate_of_change = result["极差"].pct_change().fillna(0)
    pd.testing.assert_series_equal(
        result["极差的时间变化率"], expected_rate_of_change, check_names=False, atol=1e-10
    )
...
@given(
    ...
    freq_minutes=st.sampled_from(
        [1, 5, 10, 15, 30, 60, 120]
    ),  # Different sampling frequencies in minutes
)
...
def test_property_13_empty_dataframe_consistent_error_handling(field_name, feature_list, freq):
    ...
    **Validates: Requirements 5.5**
    ...
    result = calculator.calculate_statistical_features(empty_df, field_name, feature_list, freq)
...
@given(
    ...
    feature_list=st.lists(
        st.sampled_from(["均值", "中位数", "最大值", "最小值", "标准差"]), min_size=1, max_size=3
    ),
...
def test_property_13_missing_field_consistent_error_handling(
    ...
    """
    Consistent error handling for missing fields.
    ...
    **Validates: Requirements 5.5**
    ...
    start_time = datetime(2024, 1, 1)
    index = pd.date_range(start=start_time, periods=num_rows, freq="h")
    df = pd.DataFrame({valid_field: np.random.randn(num_rows)}, index=index)

    # Request features for non-existent field
    result = calculator.calculate_statistical_features(df, invalid_field, feature_list, freq)
...
def test_property_13_stable_period_no_stable_periods_consistent_error_handling(
    num_rows, threshold
):
    """
    ...
    **Validates: Requirements 5.5**
    ...
    volatility_df = pd.DataFrame(
        {"标准差": np.random.uniform(threshold + 1, threshold + 10, num_rows)}, index=index
    )
...
def test_feature_calculator_initialization():
    ...
    assert hasattr(calculator, "_feature_calculators"), (
        "Should have _feature_calculators attribute"
    )
    assert isinstance(calculator._feature_calculators, dict), (
        "_feature_calculators should be a dict"
    )
    ...
    for feature in expected_features:
        assert feature in calculator._feature_calculators, (
            f"Feature '{feature}' should be in _feature_calculators"
        )
...
def test_feature_calculator_methods_exist():
    ...
    assert hasattr(calculator, "calculate_statistical_features"), (
        "Should have calculate_statistical_features method"
    )
    assert hasattr(calculator, "analyze_spectral"), "Should have analyze_spectral method"
    assert hasattr(calculator, "calculate_volatility_features"), (
        "Should have calculate_volatility_features method"
    )
    assert hasattr(calculator, "find_stable_period"), "Should have find_stable_period method"
    assert hasattr(calculator, "recalculate_single_window"), (
        "Should have recalculate_single_window method"
    )

    # Check methods are callable
    assert callable(calculator.calculate_statistical_features), (
        "calculate_statistical_features should be callable"
    )
    assert callable(calculator.analyze_spectral), "analyze_spectral should be callable"
    assert callable(calculator.calculate_volatility_features), (
        "calculate_volatility_features should be callable"
    )
    assert callable(calculator.find_stable_period), "find_stable_period should be callable"
    assert callable(calculator.recalculate_single_window), (
        "recalculate_single_window should be callable"
    )


# ============================================================================
# Individual MCP tool correctness
# Feature: feature-calculator-mcp
# ============================================================================
...
def test_property_16_individual_mcp_tool_correctness(num_rows, field_name, feature, freq):
    ...
    **Validates: Requirements 4.5**
    ...
    individual_result = calculator.calculate_statistical_features(
        parsed_df, field_name, [feature], freq
    )
    ...
    assert expected_column in individual_result.columns, (
        f"Individual result should have {expected_column}"
    )
...
def test_property_12_mcp_input_validation(invalid_input_type, field_name, freq):
    ...
    **Validates: Requirements 4.3**
    ...
    if invalid_input_type == "empty_json":
        ...
        df = pd.DataFrame({"other_field": np.random.randn(10)}, index=index)
...
def test_property_12_mcp_validation_prevents_processing(
    num_rows, valid_field, invalid_field, freq
):
    """
    ...
    **Validates: Requirements 4.3**
    ...
    start_time = datetime(2024, 1, 1)
    index = pd.date_range(start=start_time, periods=num_rows, freq="h")
    df = pd.DataFrame({valid_field: np.random.uniform(10, 100, num_rows)}, index=index)
    ...
    error_response = create_error_response(
        "ValidationError", error_msg, {"field_name": invalid_field}
    )
    result_data = json.loads(error_response)
...
def test_property_11_mcp_json_round_trip(num_rows, num_cols, include_special_values, timezone):
    ...
    **Validates: Requirements 4.2**
    ...
    assert reconstructed_df.index.tz is not None, "Index should be timezone-aware after round-trip"

    # Verify column names are preserved
    for col in original_df.columns:
        assert col in reconstructed_df.columns, f"Column '{col}' should be preserved in round-trip"
...
def test_property_11_mcp_json_round_trip_with_calculator(num_rows, field_name, feature_list, freq):
    ...
    **Validates: Requirements 4.2**
    ...
    # Verify structure
    assert len(reconstructed_df) == len(result_df), "Row count should be preserved in round-trip"
