...
def test_batch_with_range_calculation():
    ...
    df = parse_json_to_dataframe(data_json)
    result_df = calculator.calculate_statistical_features(
        df=df, field_name="temperature", feature_list=["最大值", "最小值"], freq="D"
    )
    result_json = dataframe_to_json(result_df)
...
def test_error_response_structure():
    ...
    assert error_obj["message"] == "Field 'temperature' not found in DataFrame", (
...
def test_missing_field_returns_error():
    ...
    dates = pd.date_range(start="2024-01-01", periods=24, freq="h", tz=timezone.utc)
    data = {"time": [d.isoformat() for d in dates], "humidity": [60 + i * 0.5 for i in range(24)]}
    data_json = json.dumps({"data": data})
    ...
    assert "error" in result_data, "Response should have 'error' field"
    assert result_data["error"]["type"] == "ValidationError", (
        "Error type should be ValidationError"
    )
    assert "temperature" in result_data["error"]["message"], (
...
def test_invalid_frequency_returns_error():
    ...
    assert "frequency" in result_data["error"]["message"].lower(), (
...
def test_invalid_feature_list_returns_error():
    ...
    result = _calculate_statistical_features_impl(
        data_json, "temperature", ["invalid_feature", "another_invalid"], "h"
    )
    ...
    assert "invalid" in result_data["error"]["message"].lower(), (
        "Error message should mention invalid features"
    )
...
def test_empty_json_returns_error():
    ...
    assert "error" in result_data, "Response should have 'error' field"
    assert result_data["error"]["type"] == "JSONParseError", "Error type should be JSONParseError"
    assert "empty" in result_data["error"]["message"].lower(), (
...
def test_error_response_with_no_details():
    ...
    error_response = create_error_response(
        error_type="InternalError", message="An unexpected error occurred"
    )
