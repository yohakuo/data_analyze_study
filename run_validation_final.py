...
def run_comparison():
    ...
    try:
        ...
        col_names = [
            'stat_id', 'temple_id', 'device_id', 'stats_start_time',
            'monitored_variable', 'stats_cycle', 'feature_key', 'feature_value',
            'standby_field01', 'created_at'
        ]

        # Load my_hour_average.csv by skipping the broken header and assigning names manually
        df_my = pd.read_csv('my_hour_average.csv', encoding='utf-8-sig', skiprows=1, names=col_names)
        ...
        df_my_clean['stats_start_time'] = pd.to_datetime(df_my_clean['stats_start_time']).dt.tz_localize('UTC')
        ...
        df_hour_clean['stats_start_time'] = pd.to_datetime(df_hour_clean['stats_start_time']).dt.tz_localize('Asia/Shanghai').dt.tz_convert('UTC')
