...
def generate_sample_data(days=30, freq='h'):
    ...
    print(f"Temperature range: {df['空气温度（℃）'].min():.2f}°C to {df['空气温度（℃）'].max():.2f}°C")
...
def example_3_volatility_features():
    ...
    if start_date is not None:
        ...
        print(f"  Average std during period: {volatility.loc[str(start_date):str(end_date), '标准差'].mean():.4f}°C")
    else:
...
def example_4_single_window_recalculation():
    ...
    print(f"Difference: {abs(batch_features.iloc[0]['均值'] - single_window_features['均值']):.6f}")
...
def example_5_mcp_server_calls():
    ...
    if "features" in mean_result:
        print(f"Successfully calculated daily mean")
    ...
    if "features" in batch_result:
        print(f"Successfully calculated multiple features")
    ...
    print(f"Serialized 10 rows to JSON")
...
def example_6_comprehensive_workflow():
    ...
    print(f"Temperature range: {daily_stats['最小值'].min():.2f}°C to {daily_stats['最大值'].max():.2f}°C")
    print(f"Average daily range: {daily_stats['极差'].mean():.2f}°C")
    ...
    if spectrum is not None:
        ...
        for idx, row in top_3_periods.iterrows():
            ...
            amplitude = row["强度(幅度)"]
            print(f"  Period: {period_hours:.1f} hours ({period_hours/24:.1f} days), Amplitude: {amplitude:.4f}")
    ...
    print(f"Least volatile day: {least_volatile_day.date()} (std: {volatility.loc[least_volatile_day, '标准差']:.4f}°C)")
